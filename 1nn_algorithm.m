...
for i=1:pixel_num
    ...
    if h_cv(1,i) ~= 0 || s_cv(1,i) ~= 0 || v_cv(1,i) ~= 0
        ...
        % (For 1NN) Find out the minimum distance
        [dist,index]=min(dist_vec);

        % (For 1NN) Classify pixels
...
ylabel('Saturation');
zlabel('Value');
%title('Results of 1-nearest neighbor (Red: Membrane. Blue: Needle. Magenta: Thread)');
grid on
%%
% Save the figure as a vector image
f=gcf;
file_path = 'E:/project_for_Matlab_and_ADS/suturing_project/check_pixel_value/vector_images/1nn/';
